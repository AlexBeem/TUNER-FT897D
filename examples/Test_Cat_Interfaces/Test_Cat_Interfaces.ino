...
int freqUpPin = 5;                          // Define rotary encoder pins.
int freqDownPin = 6;
...
float ftFreq;
uint32_t freq = 145000000;
uint32_t RftFreq;  // Частота для передачи через SetVfoFreq
uint32_t incr = 10;
...
ISR(PCINT2_vect) {
  ...
  if (result) {
    if (result == DIR_CW) {                   // Clockwise rotation so add increment to frequency
       if ((freq + incr) < 146000000){ freq+=incr; RftFreq = freq/10;}
       
    } else {
        if ((freq - incr) > 144000000){ freq-=incr; RftFreq = freq/10;} 
        }  
    }
}
*/
...
unsigned char result = r.process();
if (result) {
    if (result == DIR_CW)  { if ((freq + incr) < 146000000){ freq+=incr; RftFreq = freq/10;}}
    if (result == DIR_CCW) { if ((freq - incr) > 144000000){ freq-=incr; RftFreq = freq/10;}} 
  }
  
Radio.SetVfoFreq(RftFreq);
}
